...
async function loadTasksForDate(date) {
    ...
    try {
        ...
        if (!tasksData.study) tasksData.study = [];
        if (!tasksData.skill) tasksData.skill = [];
        if (!tasksData.necessary) tasksData.necessary = []; // Ensure necessary exists
...
async function saveTasks() {
    if (!currentUserId) {
        ...
        alert("Please log in to save your tasks."); // Let the user know
        return;
    }
...
function renderTasks(sectionType) {
    ...
    tasks.forEach((task, index) => {
        ...
        listItem.innerHTML = `
            <span class="flex-grow text-gray-800 task-text">${task.text}</span>
...
// --- Modal Functions for Reading Funnel ---

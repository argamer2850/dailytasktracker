...
/**
 * Validates if newTaskPointInput has unadded text.
 * @returns {boolean} True if input is empty or added, false if it has unadded text.
 */
function validateTaskPointInput() {
    const newPointText = newTaskPointInput.value.trim();
    if (newPointText) {
        alert("Please add the sub-point by clicking the '+' button or pressing Enter before saving or closing.");
        newTaskPointInput.focus();
        return false;
    }
    return true;
}
...
function renderTaskPoints() {
    ...
    details.forEach((point, pointIndex) => {
        ...
        pointItem.className = 'task-point-item'; // For styling and SortableJS
        pointItem.dataset.pointIndex = pointIndex; // Store original index for SortableJS
        ...
        pointTextSpan.addEventListener('click', () => editTaskPoint(pointIndex, pointTextSpan)); // Enable editing
    ...
    // Initialize SortableJS for task points
    initializeTaskPointsSortable();
}

// NEW Function: Edit individual task point text
function editTaskPoint(pointIndex, pointTextSpan) {
    const task = tasksData[currentTaskSection][currentTaskIndex];
    if (!task || !task.details || !task.details[pointIndex]) return;

    const originalText = task.details[pointIndex].text;

    const inputField = document.createElement('input');
    inputField.type = 'text';
    inputField.value = originalText;
    inputField.className = 'edit-task-input'; // Use existing edit task input styling

    // Replace the text span with the input field
    pointTextSpan.parentNode.replaceChild(inputField, pointTextSpan);
    inputField.focus();
    inputField.select();
    ...
    const saveChanges = () => {
        const newText = inputField.value.trim();
        if (newText && newText !== originalText) {
            task.details[pointIndex].text = newText;
            // No direct saveTasks() here, it will be done via saveTaskDetails()
            renderTaskPoints(); // Re-render to show updated text
        } else {
            renderTaskPoints(); // Re-render to revert to original text if empty/cancelled
        }
    };
...
/**
 * Saves task details to Firestore after validating input.
 */
async function saveTaskDetails() {
    ...
    if (!validateTaskPointInput()) {
        return; // Prevent saving if input has unadded text
    }
...
/**
 * Closes the Task Details modal after validating input.
 */
function closeTaskDetailsModal() {
    // Validate input before closing
    if (!validateTaskPointInput()) {
        return; // Prevent closing if input has unadded text
    }
...
// --- SortableJS Initialization for Main Tasks ---
function initializeSortable(sectionType) {
    ...
    Sortable.create(listElement, {
        ...
        onEnd: async function (evt) {
            ...
            // Create a new ordered array based on the DOM elements
            const newOrder = [];
            const items = listElement.children; // Get live collection of children

            for (let i = 0; i < items.length; i++) {
                const item = items[i];
                // Assuming task-text content is unique for simple identification
                // A better approach for robust systems would be unique IDs on tasks
                const text = item.querySelector('.task-text').textContent.trim();
                const originalTask = tasksData[sectionType].find(t => t.text === text);
                if (originalTask) {
                    newOrder.push(originalTask);
                }
            }
            tasksData[sectionType] = newOrder; // Update the in-memory data
            await saveTasks(); // Save the new order to Firestore
            // Re-render the specific section to ensure DOM matches data and re-attach listeners
            renderTasks(sectionType);
        }
    });
}

// NEW Function: SortableJS Initialization for Task Details sub-points
function initializeTaskPointsSortable() {
    Sortable.create(taskPointsList, {
        animation: 150,
        handle: '.task-point-text', // Drag by the text of the sub-point
        onEnd: function (evt) {
            if (!currentUserId) return;
            ...
            if (!task || !task.details || oldIndex === undefined || newIndex === undefined) return;

            // Reorder the in-memory details array
            const [movedItem] = task.details.splice(oldIndex, 1);
            task.details.splice(newIndex, 0, movedItem);

            renderTaskPoints(); // Re-render to update numbering and reflect new order
            // Saving will happen when the main 'Save' button is clicked for the modal
        }
    });
}